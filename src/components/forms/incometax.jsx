...
const IncomeForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api/submit-income-certificate', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...formData,
          files: {
            photo: formData.files.photo,
            idProof: formData.files.idProof,
            addressProof: formData.files.addressProof,
            incomeProof: formData.files.incomeProof
          }
        })
      });
