...
import { Search, Filter, Calendar, Download, Mail, Phone, MapPin, Building2, Wallet } from 'lucide-react';
...
const GSTRegistrationDashboard = () => {
  ...
  const fetchApplications = async () => {
    try {
      const response = await fetch('/api/gst-fetch');
  ...
  const filteredApplications = applications.filter(app => 
    ...
    app.bankDetails.accountNumber.includes(searchTerm)
  );
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-gray-900">GST Registration Applications</h1>
                ...
                <div className="bg-gray-50 rounded-lg p-4 mt-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-medium">{app.bankDetails.accountNumber}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">IFSC Code</p>
                      <p className="font-medium">{app.bankDetails.ifscCode}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Bank Name</p>
                      <p className="font-medium">{app.bankDetails.bankName}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Branch</p>
                      <p className="font-medium">{app.bankDetails.branchName}</p>
                    </div>
                  </div>
                </div>
...
export default GSTRegistrationDashboard;
