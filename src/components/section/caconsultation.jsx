...
export default function CAConsultationDetail({ params }) {
  ...
  const fetchConsultationDetails = async () => {
    try {
      setLoading(true)
      const response = await axios.get(`/api/ca/consultations/${consultationId}?clientType=${clientType}`)
      setConsultation(response.data)
      if (response.data.caNotes) {
        setNotes(response.data.caNotes)
      }
    } catch (error) {
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      const response = await axios.patch(`/api/ca/consultations/${consultationId}`, {
        clientType,
        status: newStatus
      })
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="mb-6 flex items-center justify-between">
          ...
          <button 
            onClick={() => window.location.href = "/ca/dashboard"}
        ...
        <div className="bg-white shadow-sm rounded-lg overflow-hidden">
          <div className="p-6 border-b border-gray-200">
                ...
                <div className="space-y-2">
                  {consultation.clientType === "Individual" ? (
                    <>
                      ...
                      {consultation.panNumber && (
                        <p><span className="font-medium">PAN Number:</span> {consultation.panNumber}</p>
                      )}
                      {consultation.occupation && (
                        <p><span className="font-medium">Occupation:</span> {consultation.occupation}</p>
                      )}
                    </>
                  ) : (
                    <>
            ...
            <div className="mb-6">
              <h3 className="text-sm font-medium text-gray-500 mb-2">CA Notes</h3>
              <textarea
                value={notes}
