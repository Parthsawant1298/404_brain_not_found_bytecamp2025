// /app/api/ca/consultations/route.js
...
export async function GET(request) {
  try {
    ...
    const individualConsultations = await IndividualConsultation.find(filter).sort({ createdAt: -1 });
    const firmConsultations = await FirmConsultation.find(filter).sort({ createdAt: -1 });
