// /app/api/ca/consultations/[id]/route.js
...
const getModelByClientType = (clientType) => {
  return clientType === 'Individual' ? IndividualConsultation : FirmConsultation;
};
...
// PATCH function to update consultation status or add CA notes
export async function PATCH(request, context) {
  try {
    ...
    const { clientType, status, caNotes } = body;
    ...
    if (caNotes !== undefined) updateData.caNotes = caNotes;
    
    // Add caNotes field to schemas if not already there
    if (!Model.schema.paths.caNotes) {
      Model.schema.add({
        caNotes: {
          type: String,
          trim: true
