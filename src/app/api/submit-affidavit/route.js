...
const validateField = (value, field, validationRules = {}) => {
  ...
  if (field === 'purpose' && value.trim().length < 10) {
    return 'Please provide a detailed purpose for the affidavit (minimum 10 characters)';
  }
...
const validateProofType = (type, validTypes) => {
  if (!validTypes.includes(type)) {
    return `Invalid proof type selected`;
  }
  return null;
};
...
export async function POST(req) {
  try {
    ...
    const validIdProofTypes = [
      'panCard', 'passport', 'aadhar', 'voter', 'driving', 
      'rsby', 'mnrega', 'govtId'
    ];
    
    const validAddressProofTypes = [
      'passport', 'ration', 'aadhar', 'phone', 'driving', 
      'electricity', 'water', 'voter', 'tax', 'rent'
    ];
    ...
    const idProofTypeError = validateProofType(body.idProofType, validIdProofTypes);
    if (idProofTypeError) errors.push(idProofTypeError);

    const addressProofTypeError = validateProofType(body.addressProofType, validAddressProofTypes);
    if (addressProofTypeError) errors.push(addressProofTypeError);
    ...
    const application = new AffidavitApplication({
